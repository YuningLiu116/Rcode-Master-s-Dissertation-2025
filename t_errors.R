...
#######################################################
#### Exact two-sided STI over [a,b] newly proposed ####
#######################################################

##############################################################
#### For each (Z,u), calculate the minimum value in (6).  ####
####  Step 1: generate a equal spaced sequence {c1,...,cp}####
####          with c1=a and cp=b. Splite [a,b] into (p-1) ####
####          small intervals, [c1,c2], [c2,c3],...,      ####
####          [c(p-1),cp].                                ####
####  Step 2: calculate the minimum value optim.i in      ####
####          [ai,bi], i=1,...,p.                         ####
####  Step 3: the minimum value in (6) is min(optim.i).   ####
#################################################################
#### In our simulation studies, the accuracy of critical     ####
####  constants is enough when the number of small intervals ####
####  is 5, and the number of elements in the equal spaced   ####      
####  sequence {c1,...,cp} is cut.p=5+1=6.                   ####
#################################################################
...
two.sti.exact=function(lam,n,p,Cbeta,a,b,B,u,invXTX,cut.p)
{
  ...
  ##  B -- the column-vector of random Z in (5)
  ##  u -- the r.v. u in (5)
...
t_cov_boot <- system.time({
  cover_B_vec <- foreach(j = 1:nsim1, .combine = "c") %dopar%{
    ...
    min.cov=function(x.new,invXTX,lambda){
      ...
      L_1=as.numeric(t(vx)%*%alp.h)-lambda*(zbeta+sqrt((p+2)*d2.x))*sig.h # The value of l(x)
      U_1=as.numeric(t(vx)%*%alp.h)+lambda*(zbeta+sqrt((p+2)*d2.x))*sig.h # The value of u(x)
      y.mean=as.numeric(t(vx)%*%alp.t) # The true mean of y_x-distribution
      # True value: E[Y|x] = x' * beta.t = y.mean
      # True standard deviation: sig.t
      t.low = (L_1 - y.mean) 
      t.upp = (U_1 - y.mean) 
      # P(y in [L, U]) = F(U) - F(L)
      p.yx = pt(t.upp, df = df) - pt(t.low, df = df)
      p.yx
    }
